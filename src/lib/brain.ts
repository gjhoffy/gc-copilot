...
async function tavilySearch(query: string, apiKey: string): Promise<{ title: string; uri: string; content: string }[]> {
  try {
...
export const runBrain = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    // Run Tavily search for all modes except framer
    ...
    for (const model of MODEL_FALLBACKS) {
      for (let attempt = 0; attempt < 2; attempt++) {
        ...
        if (res.ok) {
          ...
          // Return Tavily sources in the same shape as before
