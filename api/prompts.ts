...
- E-E-A-T proofs inline: years on tools, crew size, license #, insurance, sample project addresses (bracket placeholders OK).`,
  ...
  // FIX: Updated grounding module to match where data actually lives (user message, between --- markers)
  grounding: {
    id: "grounding",
    name: "Search Grounding",
    description: "How to use live search results",
    content: `GROUNDING:
- Live 2026 web search results are appended at the end of the user's message between --- markers.
- You MUST use them. Pull specific data points, prices, competitor names, and cite sources by URL.
- If the search results section is empty or missing, fall back to training knowledge but flag it with "(estimated — no live data)".
- Never ignore the search results. They are the most current signal available.`,
    tags: ["search", "data"],
  },
...
- [MODE: audit] — technical/SEO audit with ranked fixes.
...
7. **FRAMER CMS BLOCK** — a single \`\`\`json fenced block with keys: title, slug, excerpt, hero_alt, body_md, faq, schema_jsonld, tags, published_at, reading_time_min.
...
// Default compositions — framer mode removed
