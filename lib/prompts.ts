...
export const DEFAULT_COMPOSITIONS: Record<string, PromptComposition> = {
  base: { modules: ["identity", "voice", "locale"] },
  market: { modules: ["identity", "voice", "locale", "standards_2026", "grounding", "routing", "competitive_intel"] },
  blog: { modules: ["identity", "voice", "locale", "standards_2026", "grounding", "routing", "blog_spec", "construction_focus"] },
  page: { modules: ["identity", "voice", "locale", "standards_2026", "grounding", "routing", "local_seo"] },
  audit: { modules: ["identity", "voice", "locale", "standards_2026", "grounding", "routing"] },
  framer: { modules: ["identity", "voice", "locale", "standards_2026", "routing"] },
  chat: { modules: ["identity", "voice", "locale", "standards_2026", "grounding", "routing", "construction_focus"] },
};
...
export function composePrompt(composition: PromptComposition): string {
  ...
  const customAdditions = composition.customAdditions?.join("\n\n") ?? "";
...
export function getModulesByTags(tags: string[]): PromptModule[] {
  return Object.values(PROMPT_MODULES).filter((module) => tags.some((tag) => module.tags?.includes(tag)));
}